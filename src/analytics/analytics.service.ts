...
export class AnalyticsService {
    ...
    private formatData(data: ApiZohoAnalyticsData[]): ApiExecuteJobData[] {
        return data.map(item => ({...item,
            ...
            ValorOrig:item.vlrori ?  Number(item.vlrori) : item.vlrori,
            ValorNFs: item.valornfs ? Number(item.valornfs) : item.valornfs,
            ValorNFv:item.valornfv ? Number(item.valornfv) : item.valornfv,
            SnFnFv: item.snfnfv,
            Representante: item.representante,
            NomeCliente: item.nomcli,
            NomeResponsavel: item.nomrep,
            NumNFv:item.numnfv ? Number(item.numnfv) : item.numnfv,
            NumPedido: item.numped ? Number(item.numped) : item.numped,
            Marca: item.marca,
